...
- **No-server windows get the real error instead of a credential lecture** —
  when no API key resolves and the target is a loopback address, VSHermes now
  probes the server *before* blaming the missing key. If nothing is listening,
  you get the honest diagnosis — "Cannot reach Hermes at … — is the gateway
  running? Try: hermes gateway run" — instead of "No API key configured…",
  which suggested a key would fix a server that isn't there. A reachable
  loopback server connects keyless (loopback permits it); if it actually
  demands API_SERVER_KEY, the first API call 401s and "VSHermes: Set API Key"
  resolves it. Keyless clients also stop sending an empty Authorization header.
- **"Set API Key" no longer prompts against a dead local server** — with a
  loopback target, the command probes /health first; if no instance is
  listening it says exactly that ("No Hermes instance detected at … — start
  the gateway first: hermes gateway run") and skips the key prompt — a key
  was never the fix for a missing server. Clearing an existing key still
  works with no server running.
- **Endpoints panel connection status stays honest** — the panel's
  "connected to …" line is now refreshed on every connection transition
  (connect, failed connect, gateway died, gateway returned). Previously only
  the chat view received transitions, so the panel could keep claiming
  "● connected to …" while the gateway was dead — or stay "○ disconnected"
  after a successful endpoint switch.
- **Local-row badges stop wearing success green** — "LOCAL — ATTACH ENABLED"
  and "LEGACY KEY" were rendered in the teal "ok" color, so a dead gateway
  still read as healthy; they are mode labels (capability / key resolution),
  not verdicts, and now render neutral. Green stays reserved for real
  verdicts: key ✓ on profiles, passing Test results, the compatibility
  banner. The active row's blue border + "active" wording remain selection
  state — active ≠ reachable.
- **Compatibility Check toolbar icon → shield-check** — the header icon
  was `$(sync)`, left over from the pre-rename "Check Sync" era and
  visually indistinguishable from Refresh History (`$(refresh)`); it's
  now `$(verified)` — the verify-this-server glyph, matching what the
  check actually does.
- **Export Session toolbar icon → Markdown logo** — the generic
  `$(export)` box-arrow is now the `$(markdown)` M-glyph: the header
  button names the output format instead of leaving the tooltip to do
  all the work, and stays visually distinct from Copy Conversation
  (`$(copy)`, clipboard) beside it.
