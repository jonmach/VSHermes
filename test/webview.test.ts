...
describe('webview bundle (dist/media/chat.js)', () => {
  ...
  it('keeps the usage line after the post-run transcript re-render (host re-posts messages)', () => {
    const { dom, post } = bootWebview();
    const base = {
      type: 'state',
      connected: true,
    ...
    const messages = dom.window.document.getElementById('messages')!;
    expect(messages.querySelector('.usage-line')).not.toBeNull();
    // refreshSessionAfterRun re-posts the stored transcript after the run —
    // the webview rebuilds from these messages and must re-attach the line.
    post({
      type: 'messages',
      sessionId: 's1',
      messages: [
